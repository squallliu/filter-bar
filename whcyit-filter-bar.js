angular.module('whcyit-filter-bar', ['ionic']);
(function (angular, document) {
  'use strict';

  angular.module('whcyit-filter-bar').directive('ionFilterBar', ['$timeout', '$compile', '$window', function ($timeout, $compile, $window) {
    var filterBarTemplate =
      '<div class="filter-bar-wrapper filter-bar-{{::config.theme}} filter-bar-transition-{{::config.transition}}">' +
        '<div class="bar bar-header bar-{{::config.theme}} item-input-inset" style="padding: 0px 5px 0px 5px;">' +
          '<button class="filter-bar-cancel button back-button button-clear">' +
            '<i class="icon {{::config.back}}"></i>' +
          '</button>' +
    ...
    return {
      ...
      link: function ($scope, $element) {
        ...
        backdrop.bind('click', function (e) {
          if (e.target.className == 'scroll' || e.target == backdrop[0]) {
            cancelFilterBar();
          }
        });
        ...
        $scope.$on('$destroy', function () {
          $window.localStorage.setItem($scope.historyKey, angular.toJson($scope.histories));
          $element.remove();
...
(function (angular) {
  ...
  angular.module('whcyit-filter-bar').provider('$ionicFilterBarConfig', function () {
    var provider = this;
    ...
    function createConfig(configObj, providerObj) {
      angular.forEach(configObj, function (value, namespace) {
        if (angular.isObject(configObj[namespace])) {
          ...
          createConfig(configObj[namespace], providerObj[namespace]);
        } else {
...
(function (angular, ionic) {
  ...
  angular.module('whcyit-filter-bar').factory('$ionicFilterBar', [
    '$document',
    '$rootScope',
    '$compile',
    '$timeout',
    ...
    '$ionicScrollDelegate',
    '$window',
    function ($document, $rootScope, $compile, $timeout, $filter, $ionicPlatform, $ionicFilterBarConfig, $ionicScrollDelegate, $window) {
      var isShown = false;
      var $body = $document[0].body;
      var templateConfig = {
        theme: $ionicFilterBarConfig.theme(),
        transition: $ionicFilterBarConfig.transition(),
        back: ionic.Platform.isAndroid() ? 'ion-android-arrow-back' : 'ion-ios-arrow-back',
      ...
      function filterBar(opts) {
        ...
        var doit = opts.doSearch;
        opts.doSearch = function (key) {
          scope.addItem(key);
          scope.hideBackdrop();
